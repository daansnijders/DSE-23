...
def get_wing_mass(M_MZF,b,S,Cr_t,lambda_2_rad,n_ult):
    return( 0.0017*M_MZF*const.kg_to_lbs*(b*const.m_to_ft/cos(lambda_2_rad))**0.75*(1+(6.3*cos(lambda_2_rad)/(b*const.m_to_ft))**0.5)*n_ult**0.55*(b*const.m_to_ft*S*const.m_to_ft**2/(M_MZF*const.kg_to_lbs*cos(lambda_2_rad)*(Cr_t*const.m_to_ft)))**0.3)*0.967
    
def get_fuselage_mass(V_dive, l_h, w_fus, h_fus, S_fus):
    return 0.021*const.K_f*(V_dive*const.ms_to_kts*l_h*const.m_to_ft/(w_fus*const.m_to_ft+h_fus*const.m_to_ft))**0.5*(S_fus*const.m_to_ft**2)**1.2-735*const.kg_to_lbs

def get_horizontaltail_mass(K_h,S_h,V_dive,lambda_h_2_rad):
    return K_h*S_h*(3.18*(((S_h*const.m_to_ft**2)**0.2* V_dive*const.ms_to_kts)/(1000*(cos(lambda_h_2_rad))**0.5))-0.287)*0.967

def get_verticaltail_mass(K_v,S_v,V_dive,lambda_v_2_rad):
    return K_v*S_v*(3.18*(((S_v*const.m_to_ft**2)**0.2* V_dive*const.ms_to_kts)/(1000*(cos(lambda_v_2_rad))**0.5))-0.287)*0.967
