...
def get_wing_mass(WMZF,b,S,Cr_t,lambda_2_rad,n_ult):
    return 0.0017*WMZF*(b/cos(lambda_2_rad))**0.75*(1+(6.3*cos(lambda_2_rad)/b)**0.5)*n_ult**0.55*(b*S/Cr_t*WMZF*cos(lambda_2_rad))**0.3

def get_fuselage_mass(V_dive, l_h, w_fus, h_fus, S_fus):
    return 0.021*1.08*(V_dive*l_h/(w_fus+h_fus))**0.5*S_fus**1.2

def get_horizontaltail_mass(K_h,S_h,V_dive,lambda_h_2_rad):
    return K_h*S_h*(3.18*((S_h**0.2* V_dive)/(1000*(cos(lambda_h_2_rad))**0.5))-0.287)

def get_verticaltail_mass(K_v,S_v,V_dive,lambda_v_2_rad):
    return K_v*S_v*(3.18*((S_v**0.2* V_dive)/(1000*(cos(lambda_v_2_rad))**0.5))-0.287)
...
def get_landinggear_mass(K_gr,Ag,Bg,Cg,Dg,TOW):
    return K_gr*(Ag+Bg*TOW**0.75+Cg*TOW+Dg*TOW**1.5)
