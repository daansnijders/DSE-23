...
def get_take_off_field_length(rho, g, h_screen, MTOW, thrust_takeoff, thrust_climb_out, C_D,
    ...
    climb_angle = get_climb_gradient(2 * 0.8 * thrust_takeoff, drag_air, MTOW, g)
    #
    ...
    if h_transition < h_screen:
        climb_out_angle = get_climb_gradient(thrust_climb_out, drag_air, MTOW, g)
...
def get_m_landing(MTOW, maximum_thrust):
    ...
    m_landing = MTOW - get_fuel_flow(0.7 * maximum_thrust, 2) * minimum_time_before_landing
    return m_landing
...
def get_cruise_fuel(thrust_cruise, cruise_range, cruise_velocity, engines_used):
    fuel_flow_cruise = get_fuel_flow(thrust_cruise, engines_used)
