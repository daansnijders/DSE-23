...
def calc_V_S(W_S,C_L_max,C_D_C_L_max,rho):
    ...
    V_S = np.sqrt(2* W_S_psf / (rho_scf * C_N_max)) * ft_to_m                   # [m/s]
    return V_S
...
def calc_V_C(W_S):
    ...
    V_C = k_c * np.sqrt(W_S_psf) * ft_to_m                                      # [m/s]
...
n_lim_C,n_lim_D = calc_gust_lf(W_S[0],MAC[2],rho,H_ft,V_cruise,CL_alpha[0],g)
