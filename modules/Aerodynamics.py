...
class Lift:
    ...
    def Airfoil_lift_flaps(self):
        ...
        clalpha_land = c_prime_tot * self.C_l_alpha
        clalpha_TO   = c_prime_TO  * self.C_l_alpha
...
#        alpha = np.array([11,11.25,11.5,11.75,12,12.25,12.5,12.75,13])  * pi/180
        alpha = np.array([6,8,10,12]) * math.pi/180
...
#            plt.plot(y_b_2, c_l_i, label=str(alpha[i]*180/math.pi))
            c_l.append(c_l_i)
        
#        plt.plot(y_b_2, c_l_max, "k--", label="Section max $C_l$")
#        plt.grid(True)
#        plt.xlabel("b/2")
#        plt.ylabel("$C_l$")
...
#        plt.show
    ...
    def Wing_lift_flaps(self, delta_C_l,C_L_alpha_w,C_l_alpha,delta_C_l_max,b_slats, delta_C_l_TO, delta_C_l_max_TO):
...
#        print(np.rad2deg(alpha_0_L))
    ...
    def CL_alpha_plot(self, CL_alpha, alpha_0_L, CL_max, alpha_CL_max, delta_CL, delta_CL_alpha, delta_CL_max, CD_0, e):
        ...
        for i in range(len(alpha)): 
            ...
            if alpha[i] <= 7:
                ...
                C_D_i = CD_0 + (C_L_i**2)/(math.pi*self.A*e)
                C_L.append(C_L_i)
                C_D.append(C_D_i)
...
#                C_L_i = (CL_max + delta_CL_max) - ((CL_max + delta_CL_max) - (delta_CL_alpha * (np.deg2rad(alpha[i]) - alpha_0_L_flaps)))**2
        ...
        plt.plot(alpha, C_D)        
        plt.grid(True)
        plt.title("$C_D - \\alpha$ curve configuration 1")
        plt.xlabel("$\\alpha$ [deg]")
        plt.ylabel("$C_{D}$ [-]")
        plt.xlim(-10,15)
        plt.ylim(0,0.16)
        ...
        plt.plot(C_D, C_L)        
        plt.grid(True)
        plt.title("$C_L - C_D$ curve configuration 1")
        plt.xlabel("$C_D$ [-]")
        plt.ylabel("$C_{L}$ [-]")
        plt.xlim(0.0,0.16)
        plt.show
...
class Moment:
    ...
    def Airfoil_moment_flaps(self, cm_des_airfoil):
        ...
        dfle = 60           #DEG
        delta_cm_krueger = cmdle*c_prime_le**2*dfle + (self.x_ref + (c_prime_le-1))*self.delta_cl_krueger + cm_des_airfoil*(c_prime_le**2 -1) + 0.75*self.cl_des_airfoil*c_prime_le*(c_prime_le-1)
    ...
    def Wing_moment(self):
        ...
        delta_cm0_et = -0.005 + (-0.009 - -0.005)/5 * 3         #1/DEG, Figure 8.98
        Cm0_w_sub = ((self.A*np.cos(self.lambda_4_rad)**2)/(self.A + 2*np.cos(self.lambda_4_rad)))*(cm0_tip+cm0_root)/2 + delta_cm0_et*self.wing_twist
